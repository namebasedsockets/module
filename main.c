...
int name_send_query(const char *name, query_resolv_cb cb, void *data)
{
	...
	if (!daemon_pid) {
		...
		sname = (struct sockaddr_name *)name;
		printk(KERN_INFO "%s:message: snname_family: %d, sname_port: %d, sname_addr: %s\n", __FUNCTION__, sname->sname_family, sname->sname_port, sname->sname_addr.name);
